  ...
  class Enrollment
    ...
    def initialize(params={})
      ...
      unless consumer.is_a Knurld::Consumer && appmodel.is_a Knurld::AppModel
        raise "Refusing to create enrollment. Consumer must be a Consumer instance, and AppModel must be an Appmodel instance."
      end

      @href = execute_request(:post, "enrollments", {:consumer => consumer.href, :application => appmodel.href})["href"]
    ...
    def populate(file_url, intervals)
      unless file_url.is_a? String && intervals.is_a? Array && intervals.first.is_a? Hash
        raise "Refusing to populate enrollment. File URL must be a string, and intervals must be an array of hashes."
      end

      execute_request(:post, "enrollments/"+self.id, {"enrollment.wav" => file_url, :intervals => intervals})
    end
    ...
    def self.complete_or_status
       status = execute_request(:get, "enrollments/"+self.id)["status"]
       case status
         when "Completed"
           true
         when "Failed"
    ...
    def self.destroy
      execute_request(:delete, "enrollments"/+self.id)
    end
  end
end
